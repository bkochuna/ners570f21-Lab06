...
namespace spmv {
template <class fp_type> class JDS : public SparseMatrix<fp_type> {
private:
...
public:
  ...
  JDS(const size_t nrows, const size_t ncols)
  ...
  /*
    ...
    jdiag   : array containing jagged diagonals in succession, size non_zero in
    first row col_ind : array containing coresponding column indices, size
    non_zero in first row jd_ptr  : pointer array pointing to the beginning of
    each jagged diagonal, size nn_zero in first row
  */
  void getJDS(std::vector<size_t> &perm, std::vector<fp_type> &jdiag,
              std::vector<size_t> &col_ind,
              std::vector<size_t> &jd_ptr) const override;
};
