...
class BCRS : public SparseMatrix<fp_type>
{
    ...
    public:
        ...
        BCRS(const size_t nrows, const size_t ncols):SparseMatrix<fp_type>(nrows, ncols), _nblk(0) {};
        ...
        */
        void getBCRS(std::vector<std::vector<std::vector<fp_type> > > &vals, std::vector<size_t> &ia, std::vector<size_t> &ja, size_t &nblk) const override;
