...
namespace spmv {
template <class fp_type> class CRS : public SparseMatrix<fp_type> {
private:
...
public:
  ...
  CRS(const size_t nrows, const size_t ncols)
  ...
  */
  void getCRS(std::vector<fp_type> &vals, std::vector<size_t> &ia,
              std::vector<size_t> &ja) const override;
};
} // namespace spmv
